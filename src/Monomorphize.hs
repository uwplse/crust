...
hasTraitImpl ix traitName traitTys = ok && all checkPred preds
  where
    ...
    (ok, preds) = case mapMaybe (unifyImplTrait traitTys') candidates of
        [x] -> (True, x)
...
resolveAbstractType' ix ty = go ty
  where
    go (TAbstract absName _ tys) =
        let allImpls = assocTys ix absName
            goodImpls = mapMaybe (unifyImplType tys) allImpls
        in case goodImpls of
            [ty] -> everywhere (mkT go) ty
            [] -> error $ "no instance for " ++ absName ++ show (map (runPp . ppTy) tys)
            _ -> error $
                "multiple instances for " ++ absName ++ show (map (runPp . ppTy) tys) ++ ":" ++
                concatMap (\ty -> "\n" ++ runPp (ppTy ty)) goodImpls
...
unifyImplTrait :: [Ty] -> TraitImpl -> Maybe [Predicate]
unifyImplTrait tys (TraitImpl lps tps impl preds) = do
    tyArgs <- unifyImpl tps impl tys
